...
class OrviboS20
  ...
  def initialize(broadcast_address = DEFAULT_BROADCAST_ADDRESS, timeout = DEFAULT_TIMEOUT)
    ...
    receiveMsg do |msg, addr|
      if msg[0..1].unpackHex == MAGIC_KEY && msg[2..3].unpackSInt == GLOBAL_DISCOVERY_MSG_SIZE && msg[4..5].unpackHex == GDISCOVER_CMD
        ...
        if !outlets.has_key?("#{data.ip}")
          s = OrviboS20Outlet.new(data.mac, data.state, data.ip, data.revision)
